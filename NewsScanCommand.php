	...
	private function sendImageToVK($newInfo, $downloadPhoto) {
		...
		try {
			...
			if(!empty($resp->response->upload_url)) {
				...
				$files = array('file1' => '@'.$this->_photoPath.$downloadPhoto);
		...
		if(!empty($news)) {
			...
			foreach($news AS $k => $newElem) {
				...
				$newInfo = array(
					'id' 	=> (int) $matches[1],
					'link' 	=> $link,
					'title' => preg_replace('/\.$/','',$this->iconvDecode($a->innertext)), // декодируем и удаляем точку в конце
		...
		if(!empty($news)) {
			...
			foreach($news AS $k => $newElem) {
				...
				$newInfo = array(
					...
					'id' 	=> (int)$matches[1],
					'link' 	=> $link,
					'title' => preg_replace('/\.$/','',$this->iconvDecode($newElem->find('a',0)->innertext)), // декодируем и удаляем точку в конце
